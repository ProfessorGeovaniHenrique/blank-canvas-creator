...
export function useBreadcrumbs(): Breadcrumb[] {
  ...
  const breadcrumbs = useMemo(() => {
    ...
    const routes: Record<string, { label: string; icon?: LucideIcon }> = {
      ...
      'lexicon-setup': { label: 'Configuração de Léxico', icon: Database },
      'metadata-enrichment': { label: 'Enriquecimento de Metadados', icon: Sparkles },
      'prototypes': { label: 'Protótipos', icon: CircuitBoard },
      'developer-logs': { label: 'Dev Logs', icon: BookOpen },
      'developer-history': { label: 'Developer History', icon: BookOpen },
      'devops': { label: 'DevOps', icon: CircuitBoard },
      'devops-metrics': { label: 'DevOps Metrics', icon: CircuitBoard },
